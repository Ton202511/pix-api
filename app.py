...
MP_ACCESS_TOKEN = os.getenv("MP_ACCESS_TOKEN")  # seu token (var de ambiente no Render)
ESP_BASE = os.getenv("ESP_BASE", "")            # ex: http://192.168.0.50:80
...
def notify_esp():
    ...
    try:
        ...
        r = requests.get(url, headers=headers, timeout=6)  # corrigido: método GET
        if r.ok:
            logging.info("✅ ESP respondeu OK (%s)", r.status_code)
            return True, r.status_code
        else:
            logging.warning("⚠️ ESP respondeu erro (%s): %s", r.status_code, r.text[:200])
