...
def webhook():
    ...
    try:
        response = requests.get(payment_url, headers=headers, timeout=REQUEST_TIMEOUT)
    ...
    if payment_data.get("status") == "approved" and payment_data.get("payment_method_id") == "pix":
        ...
        for attempt in range(1, NOTIFY_RETRY + 1):
            try:
                ...
                r = requests.get(notify_url, headers=headers, timeout=REQUEST_TIMEOUT)
...
# Configura a porta dinamicamente para o Render
